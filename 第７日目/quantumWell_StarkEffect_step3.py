############################################################################
#　無限に深い量子井戸中の電子に静電場を加えたときの固有状態（ステップ３）
############################################################################
import math
import cmath
...
for i in range(DIM):
	...
	print(f'{i}番目の固有値に対応する固有ベクトルの絶対値の２乗：\n{vectors[i]*vectors[i]}')
...
for nx in range(NX+1):
	...
	for n in range(len(phi)):
		for m in range(n_max+1):
			phi[n][nx] += vectors[n][m] * verphi(m, x)

		#描画用データの整形
		phi[n][nx] = abs(phi[n][nx])**2 / (1.0 * 10**9)

#グラフの描画
plt.title("Probability density at Position")
